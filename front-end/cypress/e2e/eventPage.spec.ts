...
describe('Event page', () => {
  ...
  it('Should show event title and date range for ID 1 in locator bar', () => {
    eventPage.verifyEventLocatorBarContent('Sample-Dataset-007', 'Data from Jul 2018 - Nov 2018')
  })
  ...
  it('Should show correct number of evaluators', () => {
    cy.get('.ag-center-cols-container div[role="row"]')
      .should('have.length', eventData.evaluators.length)
  }) 
  // Fails because "categories" format does not match displayed text 
  it.skip('Should show correct evaluator value per row', () => {
    const fields = ['id', 'description', 'fields_used', 'hits', 'accounts_affected'] 
  ...
  it('Should contain the correct URL per evaluator', () => {
    cy.get('.ag-center-cols-container div[role="row"]')
      .each((row, rowIndex) => {
        ...
        cy.wrap(row).find('.ag-cell-value')
        .then(cells => {
...
describe('Event file download', () => {
  ...
  it.skip('Verify the downloaded file', () => {
    eventPage.verifyDirectSummaryDownload('Sample-Dataset-007', [
      {
        description: 'CSVs',
          accept: {
            'text/csv': ['.csv']
