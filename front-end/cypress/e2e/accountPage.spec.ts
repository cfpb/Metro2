...
import '../../src/pages/Account/Account'
const { _ } = Cypress
...
describe('Account page loader', () => {
  it('Should show a loading view while account data is fetched', () => {
    ...
    cy.intercept('GET', 'api/events/1/account/20140800499328/', { fixture: 'account' }).as('getAccount')
...
describe('Account page', () => {
  beforeEach(() => {
    cy.setCookie(PII_COOKIE_NAME, 'true')
    cy.intercept('GET', 'api/events/1/account/20140800499328/', { delay:2000, fixture: 'account' }).as('getAccount')
    cy.visit('/events/1/accounts/20140800499328')
    cy.wait(2000)    
  })
  ...
  it('Should show account details', () => {
    ...
    const detailItems = [
      ...
      { key: 'Date opened', value: '08/15/14' /*value: accountData.account_activity[0].date_open*/} 
    ]
    accountPage.verifySummary(detailItems)
  })
  it('Should show all inconsistencies found', () => {
    cy.findAllByTestId('inconsistencies').find('h2').should('have.text', 'Inconsistencies found')
    cy.get('[data-testid="inconsistencies"] > ol')
    .each((row, rowIndex) => {
      cy.wrap(row).find('li').should('contain', accountData.inconsistencies[rowIndex])
      })
  })
  it('Should have links for each evaluator', () => {
    cy.get('[data-testid="inconsistencies"] > ol')
    .find('li > a').each(($li, index) => {
      cy.get('li > a').eq(index)
      .click()      
      cy.wait(1000)
      cy.url().should('include', '/events/1/evaluators/' + accountData.inconsistencies[index])
      cy.go('back')
      })
  })
  ...
  it('Should show correct occurrences of instances in account', () => {
    cy.get('.ag-pinned-left-cols-container > div').should('have.length', accountData.account_activity.length)
    cy.get('.ag-center-cols-container > div').should('have.length', accountData.account_activity.length)    
  })
...
describe('Account data download', {viewportHeight: 1080, viewportWidth: 1920,},() => {
  beforeEach(() => {
    cy.setCookie(PII_COOKIE_NAME, 'true')
    cy.intercept('GET', 'api/events/1/account/20140800499328/', { delay:2000, fixture: 'account' }).as('getAccount')
    cy.visit('/events/1/accounts/20140800499328')
