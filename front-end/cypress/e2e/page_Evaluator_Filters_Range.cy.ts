...
describe('Evaluator page range amount filters', () => {
  ...
  it('Should apply and remove amount filters via range inputs', () => {
    ...
    evaluatorPage.interceptFilteredResults(
      ...
      { current_bal__min: 100, current_bal_max: 1000, view: 'all' }, // query params
      'evaluatorHits_8' // fixture
    )

    // Entering 100 in the current_bal_min field should show 16 results
    cy.get('#current_bal__min').type('100{enter}')
    page.hasURL('events/1/evaluators/Test-Eval-1', {
      current_bal__min: '100',
      view: 'all'
    })
    cy.wait('@currentBalMin')
    cy.get('#current_bal__min').should('have.value', 100)
    evaluatorPage.hasResultsMessage('Showing 1 - 16 of 16  results')
    table.hasRowCount(16)

    // Entering 1000 in the current_bal_max field should show 8 results
    cy.get('#current_bal__max').type('1000{enter}')
    page.hasURL('events/1/evaluators/Test-Eval-1', {
      current_bal__min: '100',
      current_bal__max: '1000',
      view: 'all'
    })
    cy.wait('@currentBalMinMax')
    cy.get('#current_bal__min').should('have.value', 100)
    cy.get('#current_bal__max').should('have.value', 1000)
    ...
    cy.get('#current_bal__max').clear()
    cy.get('#current_bal__min').clear()
    cy.get('#current_bal__max').type('{enter}')
    cy.get('#current_bal__min').should('have.value', '')
    cy.get('#current_bal__max').should('have.value', '')
    evaluatorPage.hasResultsMessage('Showing 1 - 20 of 30')
    table.hasRowCount(20)
  })
