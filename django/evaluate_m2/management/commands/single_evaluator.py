...
class Command(BaseCommand):
    ...
    """
    help =  (
        "Runs one evaluator on the dataset linked to the Metro2Event record."
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(f"No evaluator with name {eval_name}. Exiting.") from e

        evaluator.evaluators = {eval_name: func}  # Set that eval as the only one to run

        # Delete results of previous evaluator run
        logger.info(
            "Checking if EvaluatorResultSummary records exist for event ID: "
            f"{event_id}."
        )
        eval_results = EvaluatorResultSummary.objects.filter(
            event=event, evaluator__id=eval_name
        )
        ...
        logger.info(
            self.style.SUCCESS(
                f"Finished running evaluator {eval_name} for event ID: {event_id} "
                "and saving results."
            )
        )
