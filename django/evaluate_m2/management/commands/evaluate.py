...
class Command(BaseCommand):
    ...
    creates an EvaluatorResultSummary record for each evaluator that has a hit,
    and an EvaluatorResult record for each item returned from the evaluator.
    """
    help =  (
        "Starts the evaluate process on the dataset linked to the Metro2Event "
    ...
    def handle(self, *args, **options):
        ...
        logger.info(
            self.style.SUCCESS(
                f"Finished running evaluators for event ID: {event_id} and saving "
                "results."
            )
        )
