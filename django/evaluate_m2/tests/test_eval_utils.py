...
class EvaluatorUtilsTestCase(TestCase):
    ...
    def test_insert_query_creates_expected_sql(self):
        ...
        regex_check = re.compile('^\s*INSERT INTO evaluate_m2_evaluatorresult\s* \(source_record_id, date, acct_num, result_summary_id, sample\)\s*SELECT\s*parse_m2_accountactivity\.id,\s*parse_m2_accountactivity\.activity_date,\s*parse_m2_accountactivity\.cons_acct_num,\s*\d?\d, FALSE FROM parse_m2.accountactivity')  # noqa: E501
