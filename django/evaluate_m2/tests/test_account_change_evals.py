...
class AccountChangeEvalsTestCase(TestCase, EvaluatorTestHelper):
    ...
    def test_eval_account_change_id_4(self):
    # Hits when all conditions are met:
    # 1. l1_change_ind == '1'
    # 2. l1__new_id_num != id_num
        ...
        l1_activities = [
            {'id': 32, 'change_ind': '1', 'new_id_num':'B', 'new_acc_num': ''},
            {'id': 34, 'change_ind': '2', 'new_id_num':'C', 'new_acc_num': ''},
            {'id': 35, 'change_ind': '1', 'new_id_num':'D', 'new_acc_num': '35'},
        ]
        for item in l1_activities:
            l1_record(item)
        # 32: HIT, 33: NO-missing L1 segment,
        # 34: NO-l1_change_ind=2, 35: NO-l1__new_id_num='e'

        # Create the segment data
        expected = [{
            'id': 32, 'activity_date': date(2019, 12, 31),
            'cons_acct_num': '0032', 'id_num':  'a', 'l1__change_ind':'1',
            'l1__new_acc_num':'', 'l1__new_id_num': 'B'
        }]
        self.assert_evaluator_correct(
            self.event, 'AccountChange-ID-4', expected)
