...
class Command(BaseCommand):
    ...
    """
    help = (
        "For all files on an event that have an activity_date set, "
    ...
    def handle(self, *args, **options):
        ...
        for df in event.m2datafile_set.all():
            if df.activity_date:
                ...
                logger.debug(
                    f"Skipping file {df.file_name} since it already uses DOAI."
                )

        logger.info(
            self.style.SUCCESS(
                f"Finished updating Account Activity data for event: {event_id}."
            )
        )
