...
def get_activity_date_from_header(line: str):
    if re.match(header_format, line[:10]):
...
def parse_extra_segments(line: str, parsed: dict) -> dict:
    """
    ...
    if seg_length:
        parse_extra_segments(line[seg_length:], parsed)

    return parsed

def parse_line(line: str, file_record: M2DataFile, activity_date) -> dict:
    """
    ...
    """
    if re.match(trailer_format, line[:11]):
    ...
    try:
        ...
        acct_holder = AccountHolder.parse_from_segment(
            line, file_record, activity_date)
        ...
        parsed = parse_extra_segments(remaining_chars, parsed)
        return parsed
        ...
        return {"UnparseableData": UnparseableData(
            data_file=file_record,
            unparseable_line=line,
            error_description=str(e)
        )}

def parse_chunk(f: io.TextIOWrapper, chunk_size: int, file_record: M2DataFile, activity_date) -> dict:
    """
    ...
    - file_record: the M2DataFile that represents the file that the
                   line of data came from. Used as a foreign key when
                   saving the records in each line.
    - activity_date: date object, to be saved in the AccountHolder and
    ...
    while lines_parsed < chunk_size:
        ...
        line_results = parse_line(line, file_record, activity_date)
        if line_results:
...
def save_values_bulk(values: dict):
...
def parse_file_contents(file_record: M2DataFile, f: io.TextIOWrapper, file_size: int):
    """
    Parse a Metro2 file and save the records to the database.

    Inputs:
    `file_record` - instance of M2DataFile, which will act as the parent record for all
                    data records parsed from this file
    `f` - file stream of the data file to be parsed
    `file_size` - the size of the file stream in bytes
    """
    # get first line
    header_line = f.readline()
    activity_date = get_activity_date_from_header(header_line)

    # parse the rest of the file until it is done
    while f.tell() < file_size:
        values = parse_chunk(f, chunk_size, file_record, activity_date)
        save_values_bulk(values)
